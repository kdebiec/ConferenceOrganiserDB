...
AS
    BEGIN
        ...
        set @Discount = (select top 1 Discount
                            from Discounts
                            where ConferenceID = @ConferenceID
                              and DaysBeforeConference <= @TimeLeft
                            order by DaysBeforeConference - @TimeLeft DESC)

        declare @ToPayForDaysNormal money

        set @ToPayForDaysNormal = (select sum(d.Price * (1 - @Discount)) * NumberOfNormalTickets
                                    from DayReservations as dr
                                        inner join Days as d on d.DayID = dr.DayID
                                    where dr.ReservationID = @ReservationID and dr.IsCancelled = 0
                                    group by NumberOfNormalTickets)
        ...
        set @ToPayForDaysStudent = (select sum(d.Price * (1 - @Discount - @StudentDiscount)) * NumberOfStudentTickets
                                    from DayReservations as dr
                                        inner join Days as d on d.DayID = dr.DayID
                                    where dr.ReservationID = @ReservationID
                                      and dr.IsCancelled = 0
                                    group by NumberOfStudentTickets)

        declare @ToPayForWorkshops money

        set @ToPayForWorkshops = (select sum(w.Price*wr.NumberOfTickets)
                                    from DayReservations as dr
...
AS
    BEGIN
        ...
        return dbo.SumToPayOnReservation(@ReservationID) - @Payed
    END